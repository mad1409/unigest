...
export default function CalendrierAcademique({ data }) {
  ...
  const [form,    setForm]    = useState({
    titre:"", type:"rentree", dateDebut:"", dateFin:"",
    description:"", filieres:"all",
  });
  const [viewMode, setViewMode] = useState("liste"); // liste ou mois
  const [moisActif, setMoisActif] = useState(new Date().getMonth());

  function sauvegarder(evts) {
    setEvenements(evts);
    localStorage.setItem("calendrier_"+annee, JSON.stringify(evts));
  }

  function openNew() {
    setForm({ titre:"", type:"rentree", dateDebut:"", dateFin:"", description:"", filieres:"all" });
    setEditing(null);
    setModal(true);
  }
  ...
  function save() {
    ...
    if (editing) {
      sauvegarder(evenements.map(e => e.id === editing ? {...form, id:editing} : e));
    } else {
      sauvegarder([...evenements, {...form, id: Date.now()}]);
    }
    setModal(false);
  }
  ...
  function del(id) {
    ...
    sauvegarder(evenements.filter(e => e.id !== id));
  }
  ...
  // Événements du mois actif
  const evtsMois = useMemo(() =>
    evtTries.filter(e => {
      const d = new Date(e.dateDebut);
      return d.getMonth() === moisActif;
    }),
    [evtTries, moisActif]
  );
    ...
    <div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24,flexWrap:"wrap",gap:12}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setViewMode(viewMode==="liste"?"mois":"liste")} style={{
            ...
            {evtTries.filter(e => {
              ...
              return (
                <div key={e.id} style={{
                  ...
                  <div style={{fontSize:11,color:"var(--text3)",marginTop:2}}>{jr}</div>
                </div>
              );
            })}
          </div>
            ...
            {MOIS.map((m,i) => {
              ...
              return (
                <button key={i} onClick={()=>setMoisActif(i)} style={{
                  ...
                  color:moisActif===i?"#f0c040":"var(--text2)",
                  position:"relative",
                }}>
                  {m.slice(0,3)}
                  {nbEvts > 0 && (
              ...
              <div key={e.id} style={{
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:12,color:"var(--text2)",marginTop:3}}>
                    {formatDate(e.dateDebut)}{e.dateFin && e.dateFin!==e.dateDebut ? " → "+formatDate(e.dateFin) : ""}
                  </div>
                ...
                <div style={{display:"flex",gap:6}}>
                  <button onClick={()=>openEdit(e)} style={{
                    ...
                    borderRadius:6,padding:"6px 12px",fontSize:11,color:"#38bdf8",cursor:"pointer",
                  }}>Modifier</button>
                  <button onClick={()=>del(e.id)} style={{
                    background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",
                    borderRadius:6,padding:"6px 8px",fontSize:11,color:"#ef4444",cursor:"pointer",
                  }}>X</button>
                </div>
              </div>
            );
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {evtTries.length === 0 ? (
            <div style={{textAlign:"center",padding:"60px",color:"var(--text3)",
              ...
              Aucun événement — cliquez + Ajouter événement
            </div>
          ) : evtTries.map(e => {
            const t   = getType(e.type);
            const jr  = joursRestants(e.dateDebut);
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div>
                <label style={{display:"block",fontSize:11,fontWeight:700,color:"var(--text3)",
                  marginBottom:6,textTransform:"uppercase",letterSpacing:1}}>Titre *</label>
                <input style={inp} value={form.titre}
                  onChange={e=>setForm({...form,titre:e.target.value})}
                  placeholder="Ex: Rentrée académique 2025/2026"/>
              </div>
              <div>
                <label style={{display:"block",fontSize:11,fontWeight:700,color:"var(--text3)",
                  marginBottom:6,textTransform:"uppercase",letterSpacing:1}}>Type *</label>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  {TYPES_EVENEMENT.map(t => (
                    <button key={t.value} onClick={()=>setForm({...form,type:t.value})} style={{
                      ...
                      color:form.type===t.value?t.color:"var(--text2)",
                      textAlign:"left",
                    }}>{t.label}</button>
                  ))}
                </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                <div>
                  <label style={{display:"block",fontSize:11,fontWeight:700,color:"var(--text3)",
                    marginBottom:6,textTransform:"uppercase",letterSpacing:1}}>Date début *</label>
                  <input type="date" style={inp} value={form.dateDebut}
                    onChange={e=>setForm({...form,dateDebut:e.target.value})}/>
                </div>
                <div>
                  <label style={{display:"block",fontSize:11,fontWeight:700,color:"var(--text3)",
                    marginBottom:6,textTransform:"uppercase",letterSpacing:1}}>Date fin</label>
              ...
              <div>
                <label style={{display:"block",fontSize:11,fontWeight:700,color:"var(--text3)",
                  marginBottom:6,textTransform:"uppercase",letterSpacing:1}}>Description</label>
