  ...
  return (
    <div>
      ...
      <div style={{ borderBottom:"1px solid var(--border)", marginBottom:20, display:"flex" }}>
        {[{id:"calendrier",label:"📅 Calendrier"},{id:"prochains",label:"🔔 Prochains événements"}].map(t => (
      ...
      {activeTab==="prochains" && (
        <div>
          {prochains.length === 0 ? (
            <div style={{textAlign:"center",padding:40,color:"var(--text3)"}}>Aucun événement dans les 30 prochains jours</div>
          ) : prochains.map(e => {
            const t = getType(e.type);
            return (
              <div key={e.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",background:"var(--bg2)",border:"1px solid var(--border)",borderLeft:"3px solid "+t.color,borderRadius:10,marginBottom:10}}>
                <div style={{flex:1}}>
