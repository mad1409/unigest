...
// GET /api/etudiants — prof voit son annexe seulement
router.get('/', async (req, res) => {
  try {
    let query  = 'SELECT * FROM etudiants WHERE archive IS NOT TRUE ORDER BY name';
    let params = [];
    if (req.user?.role === 'professeur' && req.user.annexe_id) {
      query  = 'SELECT * FROM etudiants WHERE annexe_id=$1 AND archive IS NOT TRUE ORDER BY name';
